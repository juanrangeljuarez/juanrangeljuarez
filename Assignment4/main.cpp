...
public:
    ...
    // MOve the obj hashtable = into the "this" hashtable
    // return obj;
    ...
    //retrieve(const string& key) method  (return by value, acts as a read only retrieve)
    //operator[](const string& key) method (return by reference which can allow for modification of values)
...
private:
    ...
    //TODO: Make an array of linked lists.
    //unique_ptr<int> arr;
    //unique_ptr<list<KeyValuePair<T>>> arr;
...
HashTable HashTable<T>::HashTable() {
    //arr = std::make_unique<list<int >>(10000);
    //arr = new list<int>[10000];// <list<int >>(10000);
    // arr = std::make_unique<list<KeyValuePair<T> >>();
    //arr = std::make_unique<int[]>(12);
    //arr = new int[10000];
    //arr = std::make_unique<list<KeyValuePair<T>>>(12);
    arr = new list<KeyValuePair<T>>[NUM_BUCKETS];
}
