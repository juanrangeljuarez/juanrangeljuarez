...
public:
    ...
    void create(const string& key, const T& item);   //create(const string& key, const T& item) method for L-values
    void create(const string& key, T&& item); //create(const string& key, T&& item) method for R-values
    T retrieve(const string& key); //retrieve(const string& key) method  (return by value, acts as a read only retrieve)
    T& operator[](const string& key); //operator[](const string& key) method (return by reference which can allow for modification of values)
    bool exists(const string& key); //exists(const string& key) method (returns a boolean)
    void remove(const string& key); //remove(const string& key) method

private:

    int hash(const string& key) const;

    //TODO: Make an array of linked lists.  If it were an array of ints, it would be:
    //unique_ptr<int[]> arr;
    std::unique_ptr<list<KeyValuePair<T>>[]> arr;
    //arr = std::make_unique<int[]>(12);
    //Also remember you need to include the memory library for unique_ptr to work.

    //Also remember, it's not an array of ints, it's an array of lists of KeyValuePair of Ts.  (It's not a KeyValuePair
    //of strings and Ts, that KeyValuePair object already has built in strings).
    //You will need to make appropriate changes in this private data member area and the constructor.
};// end class HashTable

template <typename T>
void HashTable<T>::create(const string &key, const T &item) {
    auto position = hash(key);
    KeyValuePair<T> new2(key,item);
    arr[position].push_back(new2);
}

template <typename T>
void HashTable<T>::create(const string &key, T &&item) {
    auto position = hash(key);
    KeyValuePair<T> new2(key,std::move(item));
    arr[position].push_back(std::move(new2));
}

template <typename T>
HashTable<T>::HashTable() {
    arr = std::make_unique<list<KeyValuePair<T>>[]>(NUM_BUCKETS);
}
...
T HashTable<T>::retrieve(const string &key) {
    auto number = hash(key);
    for(auto iter = arr[number].begin(); iter != arr[number].end(); ++iter){
        if(iter->key == key)
            return iter->value;
    }
}
